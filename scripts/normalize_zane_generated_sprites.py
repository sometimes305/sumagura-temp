...
SRC = ROOT / "assets" / "characters" / "sword" / "zane_source"
OUT = ROOT / "assets" / "characters" / "sword"
FRAME = 128
WIDE_FRAME = 128
USE_ORIGINAL_STICK_GUIDED_RUNTIME = True
...
def remove_green(cell):
    ...
    for y in range(h):
        for x in range(w):
            ...
            greenish = g > 135 and g > r * 1.45 and g > b * 1.45
            vivid_key = g > 115 and r < 95 and b < 95
...
def prepare_cell(cell):
    ...
    cell = remove_detached_artifacts(cell)
...
def cleanup_frame(img):
    ...
    greenish = ((g > 110) & (g > r * 1.25) & (g > b * 1.25)) | ((g > 95) & (r < 100) & (b < 100))
    arr[greenish, 3] = 0

    img = remove_detached_artifacts(Image.fromarray(arr, "RGBA"), min_area=8)
    arr = np.array(img.convert("RGBA"))
    alpha = arr[..., 3] > 12
    ...
    for comp in components:
        if comp["area"] >= 10 or (components and comp is components[0]):
            for x, y in comp["pts"]:
                keep[y, x] = True
    arr[~keep, 3] = 0
    return Image.fromarray(arr, "RGBA")
...
def label_preview(sheets):
    ...
    out = OUT / "zane_chibi_preview.png"
    preview.save(out)
    return out
...
def main():
    body_src = SRC / "zane_body_only_master_v2.png"
    if not body_src.exists():
        body_src = SRC / "zane_body_only_master_v1.png"
    body = normalize_sequence(
        split_grid(body_src, 4, 4),
        target_h=100,
        target_w=114,
        frame_w=WIDE_FRAME,
        frame_h=FRAME,
    ...
    idle_frames = body[0:4]
    run_frames = body[4:8]
    jump_frames = body[8:10]
    fall_frames = [body[11], body[11]]
    neutral_body = body[12]
    side_body = body[13]
    up_body = body[14]
    down_body = body[15]

    if USE_ORIGINAL_STICK_GUIDED_RUNTIME:
        neutral_attack_frames = [neutral_body] * 4
        side_attack_frames = [side_body] * 6
        up_attack_frames = [up_body] * 5
        down_attack_frames = [down_body] * 5
        air_neutral_frames = [jump_frames[0], jump_frames[1], fall_frames[0], fall_frames[1], jump_frames[0], fall_frames[1]]
        air_side_frames = [jump_frames[0], side_body, fall_frames[0], fall_frames[1]]
        air_down_frames = [fall_frames[0], down_body, down_body, fall_frames[1], fall_frames[0]]
    else:
        neutral_attack_frames = optional_grid("zane_attack_neutral_integrated_v1.png", 2, 2, 4, target_h=104, target_w=168) or [neutral_body] * 4
        side_attack_frames = optional_grid("zane_side_attack_integrated_v1.png", 3, 2, 6, target_h=104, target_w=152) or [side_body] * 6
        up_attack_frames = optional_grid("zane_attack_up_integrated_v1.png", 3, 2, 5, target_h=108, target_w=158) or [up_body] * 5
        down_attack_frames = optional_grid("zane_attack_down_integrated_v1.png", 3, 2, 5, target_h=104, target_w=160) or [down_body] * 5
        air_neutral_frames = optional_grid("zane_air_neutral_integrated_v1.png", 3, 2, 6, target_h=108, target_w=168) or [jump_frames[0], jump_frames[1], fall_frames[0], fall_frames[1], jump_frames[0], fall_frames[1]]
        air_side_frames = optional_grid("zane_air_side_integrated_v1.png", 2, 2, 4, target_h=108, target_w=168) or [jump_frames[0], side_body, fall_frames[0], fall_frames[1]]
        air_down_frames = optional_grid("zane_air_down_integrated_v1.png", 3, 2, 5, target_h=108, target_w=168) or [fall_frames[0], down_body, down_body, fall_frames[1], fall_frames[0]]

    sheets = []
    def add(name, frames, stabilize=True, target_cx=None, target_bottom=116):
        if stabilize:
            frames = stabilize_frames(frames, target_cx=target_cx, target_bottom=target_bottom)
        sheets.append((name, write_sheet(name, frames)))

    add("chibi_idle_sheet.png", idle_frames)
    add("chibi_run_sheet.png", run_frames)
    add("chibi_attack_neutral_sheet.png", neutral_attack_frames, stabilize=False)
    add("chibi_attack_side_sheet.png", side_attack_frames, stabilize=False)
    add("chibi_attack_up_sheet.png", up_attack_frames, stabilize=False)
    add("chibi_attack_down_sheet.png", down_attack_frames, stabilize=False)
    add("chibi_jump_sheet.png", jump_frames)
    add("chibi_fall_sheet.png", fall_frames)
    add("chibi_hurt_sheet.png", [fall_frames[0], fall_frames[1], fall_frames[0]])
    add("chibi_charge_sheet.png", [idle_frames[0], idle_frames[1], idle_frames[2]])
    add("chibi_air_neutral_sheet.png", air_neutral_frames, stabilize=False)
    add("chibi_air_side_sheet.png", air_side_frames, stabilize=False)
    add("chibi_air_down_sheet.png", air_down_frames, stabilize=False)
    add("chibi_grab_sheet.png", [neutral_body, side_body, neutral_body])
    add("chibi_dodge_sheet.png", [down_body, fall_frames[1]])
    add("chibi_throw_sheet.png", [side_body, neutral_body, side_body, idle_frames[0]])
    add("chibi_ledge_sheet.png", [idle_frames[0], idle_frames[1]])

    preview = label_preview(sheets)
    print("Generated Zane sheets:")
